...
async function renderUserRoles() {
  ...
  $('#content').innerHTML = `
    <div class="toolbar"><div class="actions"><button id="addRoleBtn">Create User Role</button><button class="secondary" id="backUsersBtn">Back to Users</button></div><div class="sub">Role names are labels only. Effective access comes only from the permissions saved inside each role and each Binance-account grant.</div></div>
    <div class="report-cards wide mt">
      ${data.items.map(r => `<div class="user-card role-card">
        <div class="user-card-head"><div><b>${escapeHtml(r.name)}</b><span>${escapeHtml(r.description || '')}</span></div>${badge('Template', 'blue')}</div>
