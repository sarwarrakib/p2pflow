...
function mobileProfileDetailsHtml(data = {}, result = {}) {
  ...
  return `<section class="mobile-profile-subpage">
    ...
    <div class="mobile-profile-detail-list">
      ...
      ${hasMetric(stats.paymentMethodsCount) ? profileDetailRow('Payment Methods', profileNumber(stats.paymentMethodsCount)) : ''}
...
function profileSecurityModalHtml(data = {}) {
  return `<div class="mobile-profile-security-modal">
    <div class="mobile-profile-security-summary"><b>${escapeHtml(data.user?.name || data.user?.username || 'User')}</b><span>${escapeHtml(data.user?.role || '')} · ${escapeHtml(data.email || 'No email')}</span></div>
    <form id="securityForm" class="form-grid">
      <div class="full-row"><label>Current Password</label><input name="currentPassword" type="password" required></div>
      <div><label>New Password</label><input name="newPassword" type="password" placeholder="optional, min 8 chars"></div>
      <div><label>Confirm New Password</label><input name="confirmPassword" type="password"></div>
...
async function renderSecurity() {
  ...
  const [data, initialProfileResult] = await Promise.all([
    api('/api/me/security'),
    api(`/api/me/p2p-profile${selectedQuery}`).catch(err => ({ forbidden: true, error: err.message || 'P2P profile access is not enabled for this role.', profile: {}, credentials: [] }))
  ]);
  let p2pResult = initialProfileResult;
  ...
  const openSecurityModal = () => {
    modal('Login Security', profileSecurityModalHtml(data));
    setTimeout(() => {
  ...
  const bindProfileActions = () => {
    ...
    $('#mobileProfileSettingsBtn')?.addEventListener('click', openSecurityModal);
    ...
    $$('[data-profile-action]').forEach(button => button.onclick = () => {
      ...
      if (action === 'payment-methods') { if (canPage('accounts')) setRoute('accounts'); else showUnavailable('Payment Methods'); return; }
