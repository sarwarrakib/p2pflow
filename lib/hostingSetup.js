...
function requestOrigin(req) {
  const proto = clean(req.headers['x-forwarded-proto'] || (req.socket.encrypted ? 'https' : 'http'), 20).split(',')[0].trim() || 'http';
  const host = clean(req.headers['x-forwarded-host'] || req.headers.host || 'localhost', 255).split(',')[0].trim();
  return `${proto}://${host}`;
}
...
function createEnvValues(body, inspection, appKey, paths) {
  const mailDriver = clean(body.mailDriver || 'local', 30).toLowerCase();
  ...
  const values = {
    ...
    P2PFLOW_PUBLIC_BASE_URL: clean(body.publicBaseUrl, 500),
    P2PFLOW_MAIL_DRIVER: ['local', 'smtp', 'php', 'sendmail'].includes(mailDriver) ? mailDriver : 'local',
    P2PFLOW_MAIL_FROM: clean(body.mailFrom || body.ownerEmail, 180),
    P2PFLOW_MAIL_FROM_NAME: 'P2PFlow',
    P2PFLOW_SMTP_HOST: clean(body.smtpHost, 255),
    P2PFLOW_SMTP_PORT: String(Number(body.smtpPort || 587) || 587),
    P2PFLOW_SMTP_SECURE: body.smtpSecure === true || String(body.smtpSecure).toLowerCase() === 'true' ? 'true' : 'false',
    P2PFLOW_SMTP_STARTTLS: body.smtpStarttls === false || String(body.smtpStarttls).toLowerCase() === 'false' ? 'false' : 'true',
    P2PFLOW_SMTP_USER: clean(body.smtpUser, 255),
    P2PFLOW_SMTP_PASS: String(body.smtpPass || '')
  };
    ...
    try {
      ...
      if (req.method === 'GET' && pathname === '/setup/api/status') {
        ...
        return sendJson(res, 200, {
          ...
          defaults: {
            ...
            publicBaseUrl: existing.P2PFLOW_PUBLIC_BASE_URL || existing.CRM_PUBLIC_BASE_URL || requestOrigin(req),
      ...
      if (req.method === 'POST' && pathname === '/setup/api/save') {
        ...
        const inspection = (preliminary.existingState || importLegacy)
          ...
          : preliminary;
        const values = createEnvValues(body, inspection, appKey, paths);
...
`, { mode: 0o600 });
        const bootstrap = targetWasEmpty ? writeBootstrapFile(paths, requestOrigin(req)) : null;
        try {
