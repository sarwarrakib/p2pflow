...
class UpdateManager {
  ...
  configStatus() {
    ...
    const launcherConnected = Boolean(process.send);
    ...
    return {
      managedInstall,
      installationMode: automaticInstallReady ? 'managed' : 'direct',
  ...
  async latestRelease() {
    ...
    const release = releases.find(item => item && !item.draft && (this.allowPrerelease || !item.prerelease));
    if (!release) return null;
    const version = safeReleaseVersion(release.tag_name || release.name);
    return {
      id: release.id,
  ...
  async validateReleaseDirectory(directory, manifest) {
    ...
    for (const required of ['server.js', 'package.json', 'public/index.html', 'lib/binanceAdapter.js', 'lib/releaseIntegrity.js']) {
      if (!fs.existsSync(path.join(directory, required))) throw new Error(`Release is missing ${required}.`);
    }
    if (!fs.existsSync(path.join(directory, 'scripts', 'p2pflow-launcher.js')) && !fs.existsSync(path.join(directory, 'scripts', 'rneed-launcher.js'))) throw new Error('Release is missing the managed launcher.');
    ...
    for (const script of ['scripts/release-self-test.js', 'scripts/postgres-state-crypto-self-test.js', 'scripts/update-manager-self-test.js', 'scripts/launcher-self-test.js']) {
      if (fs.existsSync(path.join(directory, script))) await run(process.execPath, [script], { cwd: directory, env: testEnv });
    }
    await run(process.execPath, ['server.js', '--accounting-self-test'], { cwd: directory, env: testEnv });
