...
function validateGithubRequestUrl(value) {
  let parsed;
  try { parsed = new URL(String(value || '')); }
  catch { throw new Error('GitHub request URL is invalid.'); }
  if (parsed.protocol !== 'https:' || parsed.username || parsed.password || parsed.hash) {
    throw new Error('GitHub request URL must be HTTPS and must not contain credentials or a fragment.');
  }
  const host = parsed.hostname.toLowerCase();
  const allowed = new Set([
    'api.github.com', 'github.com', 'codeload.github.com', 'objects.githubusercontent.com',
    'raw.githubusercontent.com', 'github-releases.githubusercontent.com', 'release-assets.githubusercontent.com'
  ]);
  if (!allowed.has(host) && !host.endsWith('.githubusercontent.com')) throw new Error(`GitHub request refused an unexpected host: ${host}`);
  return parsed;
}
...
class UpdateManager {
  ...
  async githubFetch(url, options = {}) {
    ...
    try {
      let current = validateGithubRequestUrl(url);
      let method = String(options.method || 'GET').toUpperCase();
      let headers = { ...this.githubHeaders(options.accept), ...(options.headers || {}) };
      let response = null;
      for (let redirectCount = 0; redirectCount <= 5; redirectCount += 1) {
        response = await fetch(current.toString(), { method, headers, redirect: 'manual', signal: controller.signal });
        if (![301, 302, 303, 307, 308].includes(response.status)) break;
        if (redirectCount >= 5) throw new Error('GitHub request exceeded the redirect limit.');
        const location = response.headers.get('location');
        if (!location) throw new Error(`GitHub redirect (${response.status}) did not include a Location header.`);
        const next = validateGithubRequestUrl(new URL(location, current).toString());
        if (next.hostname.toLowerCase() !== 'api.github.com') {
          headers = { ...headers };
      ...
      if (!response) throw new Error('GitHub request did not return a response.');
      if (!response.ok) {
  ...
  async validateReleaseDirectory(directory, manifest, options = {}) {
    ...
    const runRuntimeChecks = options.runtimeChecks !== false;
    if (runRuntimeChecks) {
      ...
      // hosting, keep fresh-stage validation lightweight. Reused/prepared releases
      // can skip process-spawn checks entirely because the signed tree hash above is
      // still recomputed before activation.
  ...
  async validateInstalledRelease(itemOrDirectory, options = {}) {
    ...
    await this.validateReleaseDirectory(directory, manifest, options);
  ...
  async stageLatest() {
    ...
    if (fs.existsSync(finalDir)) {
      await this.validateReleaseDirectory(finalDir, manifest, { runtimeChecks:false });
    ...
    try {
      ...
      await this.validateReleaseDirectory(stageDir, manifest, { runtimeChecks:false });
