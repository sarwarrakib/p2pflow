...
class PostgresStateStore {
  ...
  async loadState({ seed, migrate }) {
    ...
    const historyResult = await this.pool.query(`SELECT payload, revision, checksum, app_version, schema_version, created_at AS updated_at FROM ${history} ORDER BY revision DESC LIMIT 25`);
    ...
    const candidates = [
      ...(mainRow ? [{ row: mainRow, history: false }] : []),
      ...historyRows.map(item => ({ row: item, history: true }))
    ];
    for (const candidate of candidates) {
      ...
      try {
        ...
        lastStoredError = error;
      }
    }
    if (row) {
      this.currentRevision = Number(row.revision || 0);
  ...
  async writeState(state, reason = 'state_update') {
    ...
    try {
      ...
      await client.query(`
        INSERT INTO ${history} (revision, payload, checksum, app_version, schema_version, reason, created_at)
        VALUES ($1, $2, $3, $4, $5, $6, NOW())
        ON CONFLICT (revision) DO NOTHING
      `, [revision, payload, checksum, this.appVersion, schemaVersion, String(reason || 'state_update').slice(0, 240)]);
      await client.query(`DELETE FROM ${history} WHERE revision IN (
        SELECT revision FROM ${history} ORDER BY revision DESC OFFSET $1
      )`, [this.historyLimit]);
