...
const BASE_URL = String(process.env.BINANCE_API_BASE_URL || 'https://api.binance.com').replace(/\/$/, '');
...
async function callSignedSapiPath({ apiKey, secretKey, endpointName = '', method, path, query = {}, body = null, clientType = 'web', dryRun = true, timeoutMs = 20000 }) {
  ...
  let res;
  try {
    res = await fetch(req.url, { method: req.method, headers: req.headers, body: req.body, signal: controller.signal });
  } catch (err) {
