# P2PFlow v1.5.27 — Public Launch Checklist

## Package ও deployment

- [ ] `P2PFlow_v1.5.27_UNIFIED.zip` SHA-256 verify করা হয়েছে।
...
- [ ] Schema `35` healthy।
- [ ] Browser/CDN v1.5.27 assets load করছে।
...
## v1.5.27 receive/split policy gate

- [ ] Personal/Merchant SELL receive split-এ Send Money/Cash Out charge `0`।
- [ ] Zero-balance receive wallet incoming split গ্রহণ করতে পারে।
- [ ] Payment Split requirement ON হলে final action split modal খুলে।
- [ ] Payment Split requirement OFF হলে Mark Paid/Release split popup ছাড়াই direct হয়।
- [ ] Proof Mandatory mode screenshot ছাড়া gated final action block করে।
- [ ] Proof Optional mode screenshot ছাড়াই gated final action allow করে।
- [ ] Multiple payment numbers click করলে শুধু select হয়, auto-send হয় না।
- [ ] Send Selected-এর আগে confirmation আসে।
- [ ] Multiple number send হলে chat-এ শুধু number list যায়।
- [ ] Selected number state page reload ছাড়াই update হয়।
- [ ] Multi-number split rows Label/Serial natural order-এ এবং amount পাশে দেখায়।
- [ ] Batch split-এর একটি invalid row থাকলে partial save হয় না।
