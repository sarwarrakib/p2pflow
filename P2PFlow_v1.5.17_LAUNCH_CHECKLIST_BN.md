# P2PFlow v1.5.17 Public Launch Checklist
...
- [ ] `P2PFlow_v1.5.17_UNIFIED.zip` এবং SHA-256 file একই secure source থেকে নেওয়া হয়েছে।
- [ ] `sha256sum -c P2PFlow_v1.5.17_SHA256.txt` সফল হয়েছে।
...
- [ ] Existing data থাকলে staging clone-এ schema 31 migration সফল।
...
- [ ] `/ready` returns HTTP 200, version `1.5.17`, expected schema/revision।
...
## 9. Users, roles, Payment Account permissions and multi-account RBAC
...
- [ ] Agent-কে শুধু `accounts.view` দিলে assigned payment accounts দেখা যায়, কিন্তু Add/Edit/Offline Txn action পাওয়া যায় না।
- [ ] Agent-কে `accounts.manage` দিলে Add Account, Bulk Add এবং Edit / Access কাজ করে; direct API-তেও role gate নেই।
- [ ] Agent-কে `ledger.adjust` দিলে Offline Txn এবং permitted statement adjustment কাজ করে।
- [ ] Permission সরানোর পর UI action disappear, API 403 এবং realtime access scope refresh হয়।
- [ ] Global ও per-Binance-account প্রতিটি permission row hover/focus/touch করলে পূর্ণ English/Bangla scope দেখা যায়।


## 10. Order Acceptance and assignment routing

Test Agent A (ON), Agent B (OFF), matching route এবং controlled test order ব্যবহার করুন।

- [ ] Orders list-এর উপরে Agent-এর `অর্ডার গ্রহণ: চালু/বন্ধ` button দেখা যায়।
- [ ] OFF Agent online থাকলেও নতুন order auto-assign হয় না।
- [ ] OFF Agent-কে Manager/Admin manual assign করলে request rejected হয়।
- [ ] ON Agent offline থাকলেও matching routing, amount/capacity এবং exact Binance-account permission থাকলে assignment candidate হয়।
- [ ] `orders.view` না থাকা বা exact account grant না থাকা Agent ON হলেও ওই Binance order পায় না।
- [ ] OFF অবস্থায় নতুন login/session-এর Orders page-এ `স্যার, আপনি কি অর্ডার গ্রহণ করতে চান?` popup আসে।
- [ ] `না` OFF রাখে; `হ্যাঁ` ON save করে এবং button/realtime state update করে।
- [ ] Existing assigned order switch OFF করার সঙ্গে সঙ্গে unassign হয় না; শুধু নতুন assignment বন্ধ হয়।
- [ ] Presence monitor online/idle/away/offline দেখায়, কিন্তু assignment decision Order Acceptance দিয়ে হয়।

## 11. Binance controlled validation
...
## 12. Accounting and profit exclusion
...
## 14. Browser, mobile and accessibility
...
## 15. Monitoring, load and operations
