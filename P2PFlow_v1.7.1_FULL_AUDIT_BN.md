# P2PFlow v1.7.1 Full Audit and Optimization Report

## Scope

v1.7.1 keeps the complete v1.7.0 optimization scope and adds a System Update transport hotfix: page rendering and verification-status polling no longer POST-probe `/api/session-step`; read-only polling uses GET while owner-authorized mutation traffic remains on the neutral text/plain transport. Generic timeout/network errors are no longer labeled as HTTP 403.
