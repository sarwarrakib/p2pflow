# P2PFlow v1.5.19 Manual Update Guide

এই guide existing P2PFlow installation-কে `1.5.19`-এ update করার জন্য। একই unified ZIP fresh install, manual update এবং repository source update-এ ব্যবহার করা যায়। Production `.env`, `.p2pflow`, `shared/`, database, key এবং runtime release data overwrite করবেন না।

## Version and compatibility

- Target application version: `1.5.19`
- Target database schema: `32`
- Migration type: code-only multi-account Ads payload hotfix; schema unchanged
- Minimum Node.js: `20`
...
```bash
mkdir -p /tmp/p2pflow-1.5.19
cd /tmp/p2pflow-1.5.19
sha256sum -c P2PFlow_v1.5.19_SHA256.txt
unzip -q P2PFlow_v1.5.19_UNIFIED.zip -d source
```
...
```bash
sudo mkdir -p /opt/p2pflow/releases/1.5.19
sudo rsync -a --delete /tmp/p2pflow-1.5.19/source/ /opt/p2pflow/releases/1.5.19/
sudo chown -R p2pflow:p2pflow /opt/p2pflow/releases/1.5.19
```
...
## 16. Multi-account Ads hotfix verification

Update-এর আগে দুইটি Binance account-এর নাম/credential ID, একটি ছোট test advertisement এবং প্রত্যেক account-এর P2P payment method note করে রাখুন। Update-এর পরে:

1. **P2P Profile** page থেকে প্রতিটি Binance account আলাদাভাবে Sync করুন।
2. **Ads** page-এ প্রতিটি P2P username button নির্বাচন করে Ads list Sync/Refresh করুন।
3. দ্বিতীয় account-এর একটি ছোট advertisement-এ price বা remark-এর harmless পরিবর্তন করে Update করুন।
4. একটি private draft দ্বিতীয় account-এ publish করে advertisement number পাওয়া যাচাই করুন।
5. প্রথম account-এর একই flow আবার চালিয়ে regression নেই নিশ্চিত করুন।
6. Error হলে response-এর `credentialId`, `credentialName`, `binanceCode` ও `binanceMessage` নোট করুন; secret/API key log করবেন না।

Multi-account mode-এ কোনো payment method selected account-এর P2P Profile/known Ads থেকে exact `payId`-এ resolve না হলে request fail-closed হবে। তখন ওই account-এর P2P Profile ও Ads পুনরায় Sync করে editor reopen করুন।
