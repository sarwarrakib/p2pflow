...
const PAGE_PERMISSIONS = {
  ...
  'offline-transactions': 'orders.view',
...
const NAV_MENU_GROUPS = [
  ...
  { id:'system', label:'System', icon:'system', items:[
    ...
    ['health','Health Check','health'], ['settings','Settings','settings'], ['billing','Subscription & Billing','billing'], ['super-admin','Super Admin','superadmin'], ['system-update','System Update','update']
  ]}
];
...
const NAV_ICON_SVGS = {
  ...
  billing:'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 9h18M7 14h4"/></svg>',
  superadmin:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3l7 3v5c0 5-3 8-7 10-4-2-7-5-7-10V6z"/><path d="M9 12l2 2 4-4"/></svg>',
...
const PERMISSION_LABELS = {
  ...
  'orders.manage': 'Manage order workflow and internal chat',
  ...
  'accounting.reopen': 'Reopen a closed business day',
  'activity.view': 'View user presence and activity analytics',
  'audit.view': 'View audit logs',
  'settings.manage': 'Settings manage',
  'credentials.manage': 'Binance API credentials manage',
  'users.manage': 'User add/edit/permission manage',
  'extension.manage': 'Extension Bridge manage',
  'market.view': 'View P2P Market',
  'billing.view': 'View subscription and billing',
  'billing.manage': 'Manage workspace subscription',
  'billing.invoice': 'Create and manage billing invoices',
  'billing.reconcile': 'Reconcile billing payments/webhooks',
  'superadmin.view': 'View SaaS Super Admin',
  'superadmin.manage': 'Manage SaaS tenants and plans',
...
const PERMISSION_DESCRIPTIONS = Object.freeze({
  ...
  'orders.manage': { en: 'Manage the operational order workflow such as leave/complete task, manual feedback and internal order chat. Binance orders require the same permission on the exact Binance account.', bn: 'Order leave/complete task, manual feedback ও internal order chat-এর মতো operational workflow manage করা যাবে। Binance order-এর জন্য নির্দিষ্ট Binance account-এ একই permission লাগবে।' },
  ...
  'accounting.reopen': { en: 'Reopen a previously closed business day with an audit reason so corrected entries can be posted. It does not delete the saved close history.', bn: 'Audit reason দিয়ে আগে closed করা business day reopen করা যাবে যাতে corrected entry post করা যায়। Saved close history delete হয় না।' },
  ...
  'users.manage': { en: 'Create and edit workspace users, login state and user-level permissions. Binance operational access still requires exact account grants.', bn: 'Workspace user, login state ও user-level permission create/edit করা যাবে। Binance operational access-এর জন্য তবুও exact account grant লাগবে।' },
  'extension.manage': { en: 'Manage the tenant Chrome Extension bridge, tokens, collector tasks and connection status. It does not grant unrelated Binance actions.', bn: 'Tenant Chrome Extension bridge, token, collector task ও connection status manage করা যাবে। অন্য Binance action permission এতে পাওয়া যাবে না।' },
  'market.view': { en: 'Open the P2P Market search view and use the configured documented C2C market search path. It does not grant Ads management.', bn: 'P2P Market search view খুলে configured documented C2C market search ব্যবহার করা যাবে। Ads manage permission এতে পাওয়া যাবে না।' },
  'notifications.manage': { en: 'Change notification preferences and managed notification settings allowed to the user. Mandatory security alerts cannot be disabled.', bn: 'User-এর allowed notification preference ও managed settings পরিবর্তন করা যাবে। Mandatory security alert বন্ধ করা যাবে না।' },
  'billing.view': { en: 'View the workspace plan, entitlement, invoices, payments and subscription state.', bn: 'Workspace plan, entitlement, invoice, payment ও subscription state দেখা যাবে।' },
  'billing.manage': { en: 'Manage the workspace subscription lifecycle such as checkout, cancel-at-period-end and resume where billing policy allows.', bn: 'Billing policy অনুযায়ী checkout, cancel-at-period-end ও resume-সহ workspace subscription lifecycle manage করা যাবে।' },
  'billing.invoice': { en: 'Create or manage billing invoices within the permitted billing scope. Payment reconciliation remains a separate permission.', bn: 'Allowed billing scope-এ invoice create/manage করা যাবে। Payment reconciliation আলাদা permission।' },
  'billing.reconcile': { en: 'Review and resolve payment/webhook reconciliation items and mismatches. It does not by itself grant Super Admin tenant control.', bn: 'Payment/webhook reconciliation item ও mismatch review/resolve করা যাবে। এতে Super Admin tenant control স্বয়ংক্রিয়ভাবে পাওয়া যাবে না।' },
  'superadmin.view': { en: 'View platform-wide SaaS tenant, plan, subscription and operational statistics in the Super Admin control plane.', bn: 'Super Admin control plane-এ platform-wide tenant, plan, subscription ও operational statistics দেখা যাবে।' },
  'superadmin.manage': { en: 'Manage platform tenants, plans, entitlement overrides and Super Admin lifecycle actions. This is a platform-level privileged permission.', bn: 'Platform tenant, plan, entitlement override ও Super Admin lifecycle action manage করা যাবে। এটি platform-level privileged permission।' },
  'system.update': { en: 'Stage, verify, activate and roll back signed P2PFlow application releases. Database rollback is not implied.', bn: 'Signed P2PFlow release stage, verify, activate ও rollback করা যাবে। Database rollback এতে স্বয়ংক্রিয়ভাবে হয় না।' }
});
...
function handleServerEvent(event = {}) {
  ...
  if (event.type === 'p2p.extension.data_collected' || event.type === 'p2p.extension.failed') {
    const detail = event.data && typeof event.data === 'object' ? event.data : event;
    const orderId = Number(detail.orderId || 0);
    const activeP2pModal = document.querySelector('.p2p-info-modal');
    if (activeP2pModal && orderId && Number(state.currentOrder?.id || 0) === orderId) {
      clearTimeout(state.p2pExtensionRefreshTimer);
      state.p2pExtensionRefreshTimer = setTimeout(async () => {
        if (!document.querySelector('.p2p-info-modal')) return;
        try {
          const latest = await api(`/api/orders/${orderId}`, { silent:true, navigationScoped:false });
          if (!document.querySelector('.p2p-info-modal')) return;
...
function visiblePages() {
  return pages.filter(([id]) => {
    if (!hasPerm(PAGE_PERMISSIONS[id])) return false;
    if (id === 'system-update' && !(state.user?.isOwner === true || state.user?.isSuperAdmin === true)) return false;
    if (id === 'super-admin' && state.user?.isSuperAdmin !== true) return false;
    return true;
  });
}
...
function pageModuleUrl(page) {
  ...
  return filename ? `/js/pages/${filename}?v=${encodeURIComponent(String(state.bootstrap?.settings?.applicationVersion || '2.0.8'))}` : '';
}
...
const PAGE_RUNTIME = Object.freeze({
  ...
  billing: { render:() => renderBilling() },
  'super-admin': { render:() => renderSuperAdmin() },
...
function defaultPermissionsForSystemRole(role='agent') {
  ...
  return ['orders.view','orders.manage','orders.split','orders.final_action','binance.chat','p2p.profile.view','ads.view','ads.manage','accounts.view','accounts.use','accounting.view'].filter(permission => all.includes(permission));
}
...
function binanceCredentialPermissionMatrix(selectedRows = []) {
  ...
  const groups = Array.isArray(state.binanceAccountPermissionGroups) && state.binanceAccountPermissionGroups.length
    ...
    : [
        { id: 'orders', label: 'Orders', permissions: ['orders.view','orders.create','orders.manage','orders.assign','orders.split','orders.final_action','orders.quick_release'] },
...
async function pollP2pExtensionResult(orderId, taskId) {
  // SSE is the primary completion path. This bounded poll is only a safety net
  // for proxies/browsers that temporarily lose the realtime connection.
  const maxAttempts = 12;
  for (let i = 0; i < maxAttempts; i += 1) {
    await new Promise(r => setTimeout(r, state.realtimeConnected ? 5000 : 3000));
