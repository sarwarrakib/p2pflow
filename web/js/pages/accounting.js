...
function accountingStartRefresh(pageId, renderer) {
  ...
  }, state.realtimeConnected ? 30000 : 5000);
}
...
function accountingCarryoverReconcileButtonHandler(renderer) {
  const button = $('#accountingCarryoverReconcileBtn');
  if (!button) return;
  button.onclick = async event => {
    const current = event.currentTarget;
    current.disabled = true;
    current.textContent = 'Reconciling...';
    try {
      const result = await api('/api/accounting/reconcile-carryover', { method:'POST', body:'{}' });
      const suffix = result.legacyAgentHistoryIncomplete ? ' Legacy close snapshots still have incomplete per-agent history.' : '';
      notify(`Carryover reconciliation complete.${suffix}`, result.legacyAgentHistoryIncomplete ? 'warning' : 'ok');
      await renderer();
    } catch (error) {
      ...
      current.textContent = 'Reconcile Carryover';
    }
  };
}
...
async function renderAccounting(options={}) {
  ...
  try {
    ...
    $('#content').innerHTML = `
      ...
      ${accountingFilterToolbar(pageId, data, `${p.canManage ? '<button id="accountingSettingsBtn" class="secondary">Settings</button>' : ''}${p.canSync ? '<button id="accountingSyncBtn" class="secondary">Sync All Binance</button>' : ''}`)}

      ${!isAgent ? `<div class="accounting-kpi-grid realtime mt">
        ${accountingMetric('Binance Asset (Order Projection)', `${accountingNumber(s.ownerActualBinanceUsd,8)} ${escapeHtml(data.settings.cryptoAsset)}`, `${binance.successfulCredentialCount || binance.accounts?.length || 0} connected account(s) synced`, 'crypto')}
      ...
      ${!isAgent ? `<div class="accounting-main-grid mt">
        <section class="card accounting-panel">
          ...
          <div class="accounting-result-list">
            ...
            <div><span>Binance Order-Ledger Projection</span><b>${accountingNumber(s.ownerActualBinanceUsd,8)} USDT</b></div>
...
async function renderAccountingClosing(options={}) {
  ...
  try {
    ...
    $('#content').innerHTML = `
      ...
      ${accountingFilterToolbar(pageId, data, `${p.canManage ? '<button id="accountingSettingsBtn" class="secondary">Settings</button>' : ''}${p.canSync ? '<button id="accountingSyncBtn" class="secondary">Sync All Binance</button>' : ''}${p.canReconcileCarryover ? '<button id="accountingCarryoverReconcileBtn" class="secondary">Reconcile Carryover</button>' : ''}${p.canClose ? '<button id="accountingCloseBtn" class="dark">Close Day</button>' : ''}`)}
      <div class="accounting-kpi-grid realtime mt">
        ${accountingMetric('Actual Business Asset', `${accountingNumber(s.ownerActualBusinessAssetUsd,8)} USDT`, 'Before individual-only profit exclusion', 'crypto')}
        ${accountingMetric('Individual-only Profit Excluded', `${accountingNumber(s.excludedUserProfitUsd,8)} USDT`, accountingMoney(s.excludedUserProfitBdt), 'cash')}
        ...
        ${accountingMetric('Carryover Adjustment', `${accountingNumber(s.carryoverAdjustmentUsd,8)} ${escapeHtml(data.settings.cryptoAsset)}`, s.carryoverAgentAllocationIncomplete ? 'Legacy agent allocation incomplete' : 'Posted on settlement business date', accountingProfitClass(s.carryoverAdjustmentUsd))}
      </div>
      <div class="accounting-main-grid mt">
        <section class="card accounting-panel">
    ...
    accountingCarryoverReconcileButtonHandler(renderAccountingClosing);
