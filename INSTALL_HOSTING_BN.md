# P2PFlow v1.0.167 — একবার Setup, এরপর Owner-only Update
...
`P2PFlow_v1.0.167_HOSTING_MIGRATION.zip`-এর ভেতরের সব content সরাসরি বর্তমান Node Application Root-এ extract/copy করে code file overwrite করুন। অতিরিক্ত `P2PFlow` parent folder রাখবেন না। এই package নতুন `.env` বা database data দেয় না, তাই saved key এবং database configuration overwrite হবে না।
...
পুরোনো setup code one-time code; নতুন server instance অন্য code তৈরি করলে পুরোনোটি গ্রহণ না করা স্বাভাবিক। v1.0.167-এ code stable Application Root-এ থাকে এবং setup সফল হওয়ার পরে আর প্রয়োজন হয় না।
...
`P2PFlow_v1.0.167_GITHUB_SOURCE.zip` extract করে তার content private repository-তে upload/push করুন। Included workflow `.github/workflows/release.yml` main/master-এ নতুন version push হলে স্বয়ংক্রিয়ভাবে:
...
Current server 1.0.167 হলে 1.0.167 release update হিসেবে দেখাবে না। পরবর্তী source package 1.0.168 GitHub Desktop থেকে push করলেই workflow নিজে `v1.0.168` tag ও signed release publish করবে; তারপর Control Panel-এ update দেখাবে।
...
New release ready না হলে stable launcher আগের code release-এ ফিরে যায়। Database পুরোনো snapshot দিয়ে replace করা হয় না, তাই update-এর পরে commit হওয়া transaction code rollback-এর কারণে কাটা যায় না। Rollback শুধু compatible managed code release-এর মধ্যে অনুমোদিত।

v1.0.167 থেকে managed release history শুরু হয়। ভবিষ্যতে 1.0.167 install করলে 1.0.167-এ code rollback করা যাবে। Buggy v1.0.163-এ automatic rollback ইচ্ছাকৃতভাবে দেওয়া হয়নি।
...
- Node child process/fork চালানোর অনুমতি
- Linux `tar` utility
- private GitHub API outbound HTTPS access

System Update page-এর Setup Status-এ launcher/repository/token/signing key সব Ready না হলে Install button চালু হবে না।
